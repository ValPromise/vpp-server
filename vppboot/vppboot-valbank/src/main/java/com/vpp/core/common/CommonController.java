...
public class CommonController {
	...
	public static final String VALBANK_LOGIN_KEY = "valbank_login_key_";
	
	public static final int VALBANK_LOGIN_CACHE_THIRTY_MINUTE = 60*30; //登录缓存30分钟

	public static final String VALBANK_MOBILE_CODE = "valbank_mobile_code_";

	public static final int MOBILE_CODE_CACHE_THIRTY_MINUTE = 60*5;// 手机验证码缓存5分钟
	...
	public String setLoginToken(String id){
		  ...
		  String setToken = VALBANK_LOGIN_KEY+token;
		  try {
			  loginTokenRedis.opsForValue().set(setToken,id,VALBANK_LOGIN_CACHE_THIRTY_MINUTE,TimeUnit.SECONDS);
		  } catch (Exception e) {
	...
	public boolean checkLogin(String token) {
		...
		try {
		      customerId = loginTokenRedis.opsForValue().get(VALBANK_LOGIN_KEY+token);
		      if(StringUtils.isEmpty(customerId)){
					 return false;
			  }
		      loginTokenRedis.opsForValue().set(VALBANK_LOGIN_KEY+token,customerId,VALBANK_LOGIN_CACHE_THIRTY_MINUTE,TimeUnit.SECONDS);
		} catch (Exception e) {
	...
	public String setLoginToken(String mobile,String imei,String id){
		...
		loginTokenRedis.opsForValue().set(VALBANK_LOGIN_KEY+mdMobile, mdImei);
	...
	public boolean checkLoginToken(String token) {
		...
		String resultImei = loginTokenRedis.opsForValue().get(VALBANK_LOGIN_KEY+mdMobile);
	...
	public void setMobileCodeRedis(String mobile,String code){
		mobileCodeTokenRedis.opsForValue().set(VALBANK_MOBILE_CODE+mobile+code,code,MOBILE_CODE_CACHE_THIRTY_MINUTE,TimeUnit.SECONDS);
	}
	...
	public boolean checkMobileCode(String mobile,String code) {
		String getCode =VALBANK_MOBILE_CODE+mobile+code;
		if (mobileCodeTokenRedis.opsForValue().get(getCode) == null) {
			return false;
		}
		return true;
	}
	...
	public boolean delLoginToken(String token){
		try {
			loginTokenRedis.delete(VALBANK_LOGIN_KEY+token);
		} catch (Exception e) {
	...
	public boolean loginOut(String token,String mobile){
		...
		try {
			...
			String mdMobile = arry[0];
			String mdImei = arry[1];
			loginTokenRedis.delete(VALBANK_LOGIN_KEY+mdMobile);
